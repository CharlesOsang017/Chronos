...
export const useWatchTaskMutation = () => {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: (data: { taskId: string; watch: boolean }) => postData(`/tasks/${data.taskId}/watch`, {}),
        onSuccess: (data: any) => {
            queryClient.invalidateQueries({
                queryKey: ['task', data._id],
            }),
...
export const useArchiveTaskMutation = () => {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: (data: { taskId: string; archive: boolean }) => postData(`/tasks/${data.taskId}/archive`, {}),
        onSuccess: (data: any) => {
            queryClient.invalidateQueries({
                queryKey: ['task', data._id],
            }),
